...
out_filename="${out_dir}export_${current_commit}.gif"
...
ffmpeg -f image2 -framerate 30 -i "$frame_pattern" -i $palette -filter_complex "[0:v][1:v] paletteuse" -y "${out_filename}"
